...
const faqs = [
  ...
  {
    q: "Как туда долететь?",
    a: "Самый удобный вариант — прямые рейсы «Аэрофлота» из Москвы в Дели. Стоимость билета в одну сторону — около 40 000 руб. В Дели мы встретим вас и организуем совместный трансфер в горы.",
  },
  {
          ...
          <Accordion type="single" collapsible className="space-y-2">
            {faqs.map((faq, i) => (
              <AccordionItem key={i} value={`item-${i}`} className="border border-border rounded-lg px-6 bg-card">
                <AccordionTrigger className="font-display text-lg text-foreground hover:text-primary py-5 text-left md:text-base">
                  {faq.q}
                </AccordionTrigger>
